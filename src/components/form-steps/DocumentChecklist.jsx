...
const DocumentChecklist = ({formData, updateFormData}) => {
  ...
  const getCategoryColor = (category) => {
    switch (category) {
      case 'medical': return 'border-indigo-200 bg-indigo-50';
  ...
  const handleDownloadChecklist = async () => {
    try {
      await generateChecklistPDF(formData, documents);
  ...
  return (
    ...
    >
      ...
      <div className="bg-white border border-gray-200 rounded-lg p-4 mb-6">
        <div className="flex items-center gap-3">
          <div className={`w-10 h-10 rounded-full flex items-center justify-center ${
            allRequiredChecked ? 'bg-green-500' : 'bg-indigo-500'
          }`}>
            <SafeIcon 
              icon={allRequiredChecked ? FiCheckCircle : FiInfo} 
              className={`text-lg ${allRequiredChecked ? 'text-white' : 'text-white'}`} 
            />
          </div>
          <div>
      ...
      {/* Missing requirements indicator - Mostrar primero si no está completo */}
      ...
      {/* Aviso sobre documentos necesarios - Solo mostrar si no están todos marcados */}
      {!(allRequiredChecked && signatureOption) && (
        <div className="bg-amber-50 border border-amber-200 rounded-lg p-6 mt-8">
          <div className="flex items-start gap-3">
            <SafeIcon icon={FiInfo} className="text-amber-600 text-xl flex-shrink-0 mt-1" />
            <div>
              <h3 className="font-semibold text-amber-900 mb-2">¿No tienes todos los documentos listos?</h3>
              <p className="text-amber-800 text-sm mb-4">
                Para avanzar en el proceso de reclamo necesitas tener listos los documentos anteriores para subirlos a la plataforma. 
                Si aún no los tienes todos, puedes descargar un checklist en PDF y regresar cuando lo hayas completado.
              </p>
              <motion.button
                whileHover={{scale: 1.02}}
                whileTap={{scale: 0.98}}
          ...
          >
            <div className="text-center">
              ...
              <p className="text-gray-600 mb-6">
                Tu checklist personalizado ha sido descargado. Úsalo para preparar todos los documentos necesarios 
                y regresa cuando estés listo para continuar con tu reclamo.
              </p>
              <div className="space-y-3">
                <motion.button
                  ...
                  className="w-full bg-[#204499] hover:bg-blue-700 text-white py-2 px-4 rounded-lg font-medium transition-colors"
                >
                  Entendido
                </motion.button>
                <p className="text-xs text-gray-500">
                  Puedes regresar a este formulario en cualquier momento para continuar tu reclamo.
                </p>
              </div>
            </div>
          </motion.div>
